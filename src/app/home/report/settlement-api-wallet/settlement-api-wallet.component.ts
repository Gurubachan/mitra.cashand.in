...
import { WalletResponse } from '../../../@model/api_settlement_wallet/WalletResponse';
@Component({
  selector: 'ngx-settlement-api-wallet',
  templateUrl: './settlement-api-wallet.component.html',
  styleUrls: ['./settlement-api-wallet.component.scss']
})
export class SettlementApiWalletComponent implements OnInit {
transactions: WalletResponse ;
...
setMaxdate(e) {
  ...
  getWalletReport(){
    ...
    this.http.post("api_client/report",data).subscribe((res: WalletResponse)=>{
      if(res.response){
